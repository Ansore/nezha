...
pre_check() {
    ...
    Get_Docker_Argu=" "
    Docker_IMG="ghcr.io\/naiba\/nezha-dashboard:latest"
}
...
update_script() {
    ...
    curl -sL https://${GITHUB_RAW_URL}/script/nezha_en.sh -o /tmp/nezha.sh
...
install_agent() {
    ...
    if [ -z "$CN" ]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/Ansore/nezha-agent/releases/download/${_version}/nezha-agent_linux_${os_arch}.zip"
    else
        NZ_AGENT_URL="https://${GITHUB_URL}/Ansore/nezha-agent/releases/download/${_version}/nezha-agent_linux_${os_arch}.zip"
    fi
...
update_docker_compose_image() {
    ...
    if grep -q "ghcr.io/naiba/nezha-dashboard$" "$yaml_file_path"; then
        sed -i 's|ghcr.io/naiba/nezha-dashboard$|ghcr.io/naibahq/nezha-dashboard:latest|' "$yaml_file_path"
    fi
}
