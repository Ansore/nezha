...
pre_check() {
    ...
    GITHUB_RAW_URL="raw.githubusercontent.com/ansore/nezha/refs/heads/main"
...
install_agent() {
    ...
    local version=$(curl -m 10 -sL "https://api.github.com/repos/Ansore/nezha-agent/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    ...
    if [[ -z $CN ]]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/Ansore/nezha-agent/releases/download/${version}/nezha-agent_darwin_${os_arch}.zip"
    else
        NZ_AGENT_URL="https://${GITHUB_URL}/Ansore/nezha-agent/releases/download/${version}/nezha-agent_darwin_${os_arch}.zip"
    fi
